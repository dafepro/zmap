...
Walking is a brisk **2.2 m/s** and sprinting is **5.4 m/s**, exported as `WALK_SPEED` and `SPRINT_SPEED`. The earlier 4 m/s default was a running pace; the new split gives the referenced walk and run animations distinct, readable timing. Hold Shift while the world canvas has keyboard focus, or call `world.setSprinting(true)` for custom controls. `world.sprinting` reports the held intent; release with `setSprinting(false)`. Sprint uses the same normalized direction, obstacle volume, slope support, input expiry and action movement locks as walking. It adds no stamina, inventory rules or avatar size advantage.

Action Yard and the courtyard expose an accessible Sprint toggle for touch or pointer control. Action Yard's toggle works with both paths and the joystick. Stop, arrival, leaving the world, a suspended input mode, Escape and loss of focus clear the appropriate held input. A manual direction change preserves the current sprint choice. A route brakes using distance in metres per second, so sprinting cannot circle or overshoot a small waypoint. Predicted arrival waits for the shared checkpoint before discarding the destination; a host handoff during the final steps can therefore resume the route.
