...
Maps can also declare `actionCatalog.performance` with approved emote IDs, exact durations and draw/stow timing. `world.emote(id | null)` plays or cancels a bounded shared expression; `world.setToolDrawn(boolean)` changes presentation while retaining the selected item. Tool use waits for drawing to finish. Enabled rooms require `performance-v1`. Character and scenery contexts additionally supply `presentationTick`, a fractional display time interpolated only between accepted checkpoints; use it to sample animations smoothly without extrapolating during a host stall. See [shared expressions and equipment presentation](shared-performances.md).
...
`createRoomService({ server, map, catalog, store, authenticate, canAccess, allowedOrigins?, capacity?, leaseMs?, adapterTimeoutMs? })` attaches `/room` to an HTTP server. `authenticate(credential, room)` returns the app's safe `Identity` or null. `canAccess(identity, room)` is rechecked during activity and periodic liveness checks. The exact authenticated identity object is retained privately for continuing `canAccess` checks, allowing adapters to associate a per-session grant with a `WeakMap`. Only the bounded public identity fields are projected to the roster or durable command adapter. Never put credentials in public identity fields.

Read-only callbacks (`authenticate`, `canAccess`, and `store.load`) time out after `adapterTimeoutMs` (default 2,000 ms). The adapter must also bound its own work; timing out the wait does not cancel an external request. Set an explicit origin allowlist and terminate TLS at the app boundary.
...
Current clients and relays negotiate `input-ack-v1` and `sprint-v1` explicitly in join/welcome. The sprint capability includes the shared 2.2 m/s walking and 5.4 m/s sprinting tuning. A room uses one movement protocol; mixing pre-sprint and sprint clients, or old and acknowledgement-capable clients, is rejected in either join order with a clear terminal protocol error. Sprint requires acknowledgement support. Deploy the matching browser bundle and relay together. A stale development relay must be restarted before opening the new client.
...
A roster update in the current host epoch merges membership into the running simulation. It must not replace the host's newer live poses, toy state or consumed input acknowledgements with an older relay checkpoint. Epoch changes still resume from the relay's accepted checkpoint. This distinction prevents joins and departures from rewinding moving players or replaying released input.
...
Kick presentation (0.1.6): `Body.kick` is optional seconds remaining in a half-second kick pose. It is replicated, bounded by snapshot validation, advances with movement prediction, and interpolates down without delaying a new kick. Apps can animate their own rigs; ball impulse rules are unchanged.
