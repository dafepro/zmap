...
| Avatar invariants       | 33 passing tests: all 5,184 structural combinations, pinned GLBs, strict shared skin/PNG contracts, actual exported shoulder deformation, illustrated shading, bounded directional capture, atomic races, independent bitmap/skeleton ownership, prepared factories and retry.                             |
| Avatar browser journeys | 16 passing tests: all eight categories, colors/history/saved looks, import/export/PNG, failed load/retry, portrait/reduced motion, 40 replacements, illustrated shader/GPU deformation, sixteen-view capture/export/cancellation, pending-choice composition, unavailable storage and empty saved lineups. |
...
| Art source              | Editable Blender kit and reproducible builder; interactive side/back/wave review and real studio screenshots retained. All 22 modular GLBs together are 1,426,104 bytes before HTTP compression; every equipped combination remains below 14,000 source triangles and 1.5 MB.                              |
...
The art approximates the supplied study's angular athletic style but is simpler than its illustrated detail. Body and tops now use connected, smoothly weighted surfaces; natural fingers and painted facial expressions replace the most conspicuous primitives. More deliberate hair masses, distinct head shapes, asymmetry, expressive hand poses, facial animation and production motion remain art-development work. The 16-view experiment captures one pose and elevation; animated sprite sheets and cloth simulation are not implemented.
